...
{
GPUBufferHandle VulkanBackend::CreateBuffer(const BufferDesc& desc,
                                            void** stagingMemory)
{
    VulkanBuffer result{};
    ...
    VK_CHECK_RESULT(vmaCreateBuffer(m_device.m_allocator, &bufferInfo,
                                    &allocInfo, &result.buffer,
    ...
    vmaGetAllocationMemoryProperties(m_device.m_allocator, result.allocation,
                                     &memFlags);
    if (!(memFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT))
    {
        LOG_WARN("Allocated non host coherent CPU-to-GPU buffer: {}",
                 (void*)result.buffer);
    }
...
void VulkanBackend::DestroyBuffer(GPUBufferHandle handle)
{
    ...
    {
        vmaDestroyBuffer(m_device.m_allocator, buffer->buffer,
                         buffer->allocation);
...
void VulkanBackend::FlushStagingBufferRanges(
    const std::span<GPUBufferSpan> ranges)
{
    ...
    for (auto& range : ranges)
    {
        ...
        VkMemoryPropertyFlags memFlags;
        vmaGetAllocationMemoryProperties(m_device.m_allocator,
                                         buffer->allocation, &memFlags);
    ...
    if (allocs.size() > 0)
    {
        vmaFlushAllocations(m_device.m_allocator, allocs.size(), allocs.data(),
