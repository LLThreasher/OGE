...
void TerrainPass2::onAttach(InitDrawContext& ctx)
{
    ...
    blockTexture = ctx.assets.backend.AllocateGPUTexture(
        BLOCK_TEXTURE_SIZE, BLOCK_TEXTURE_SIZE, 256);
    auto invalidBlockTextureBlob =
        ctx.assets.assetManager.LoadTexture("invalid.png");
    assert(invalidBlockTextureBlob);
    for (uint32_t i = 0; i < 256; ++i)
    {
        ctx.assets.streamingManager.UploadTexture<UploadType::Immediate>(
            invalidBlockTextureBlob->data,
            TextureTarget{
    ...
    // ctx.assets.streamingManager.UploadTexture<UploadType::Immediate>(tex->data,
    // blockTexture, 0);
...
void TerrainPass2::UpdateBlockTexture(AssetContext& assets,
                                      const std::string& id, uint32_t slot)
{
    assets.streamingManager.UploadTexture<UploadType::Immediate>(
        assets.assetManager.LoadTexture(id)->data,
        TextureTarget{blockTexture,
                      {0, 0, BLOCK_TEXTURE_SIZE, BLOCK_TEXTURE_SIZE},
                      slot});
}

GPUBindingGroupHandle TerrainPass2::GetOrCreateBindingGroup(
    IGraphicsBackend& backend, FrameArena& arena, GPUBufferHandle storageBuffer,
    uint32_t chunkSize)
{
    ...
    else
    {
        ...
        auto [newIt, inserted] = cachedBindingGroups.emplace(
            storageBuffer, backend.CreateBindingGroup(desc));
        return newIt->second;
    }
}
...
void TerrainPass2::onUpdate(DrawContext& ctx, View view,
                            const math::mat4& pvTransform)
{
    ...
    for (auto& cmd : view.Get<CmdDrawTerrainMeshOpaque>())
    {
        ...
        auto group = GetOrCreateBindingGroup(ctx.backend, ctx.uniformArena,
                                             resolved.buffer, resolved.size);
    ...
    for (auto [bindingGroup, val] : activeChunkSlots)
    {
        ...
        for (auto mesh : val)
        {
            ...
            auto model = math::translate(
                math::mat4(1.0f),
                math::vec3(coord.x * CHUNK_SIZE_X, coord.y * CHUNK_SIZE_Y,
        ...
        auto uniformMemory =
            ctx.uniformArena.Allocate(sizeof(UBO) * ubos.size());
        std::memcpy(uniformMemory.cpuPtr, ubos.data(),
                    sizeof(UBO) * ubos.size());
