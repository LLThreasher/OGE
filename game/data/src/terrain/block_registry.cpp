...
BlockConfig::BlockConfig(std::string blockDisplayName, std::string textureId,
                         uint32_t blockFlags, std::vector<AABB> aabb)
...
void BlockRegistry::RegisterBlock(std::string blockIdName, BlockConfig config)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        if (it == m_blockTextureIds.end())
        {
            auto [newIt, succ] = m_blockTextureIds.insert_or_assign(
                id, m_blockTextureArray.size());
            it = newIt;
            m_blockTextureArray.push_back(id);
        }
        m_textureSlots[m_nextIdx][i] = it->second;
    }
    auto it = std::search(m_aabbs.begin(), m_aabbs.end(), config.aabbs.begin(),
                          config.aabbs.end());
    ...
    else
    {
        ...
        m_aabbLookup[m_nextIdx] = std::span<AABB>(
            m_aabbs.end() - config.aabbs.size(), config.aabbs.size());
    }
...
uint16_t BlockRegistry::GetBlockId(uint32_t blockValue)
{
    return blockValue & BLOCK_ID_MASK;
}
...
const std::array<uint8_t, 6>& BlockRegistry::GetTextureSlot(
    const uint16_t blockIdx) const
{
    return m_textureSlots[blockIdx];
}

AABBList BlockRegistry::GetBlockAABBList(uint16_t blockIdx) const
{
    return m_aabbLookup[blockIdx];
}

AABBList BlockRegistry::GetDefaultBlockAABBList()
{
    return std::span<const AABB>(&DEFAULT_BLOCK_AABB, 1);
}
