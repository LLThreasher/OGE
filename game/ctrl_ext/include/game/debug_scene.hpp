...
{
public:
    class DebugScene3Instance : public GraphicalScene::Instance
    {
        entt::entity m_cross;
       ...
       public:
        DebugScene3Instance(DebugScene3& scene, const json::Value& args,
                            OGEContext& ctx)
            : GraphicalScene::Instance(scene, args, ctx)
        {
            ...
            auto& blks = m_world.ctx()
                             .get<terrain::BlockRegistry>()
                             .GetBlockTextureArray();
            for (size_t i = 0; i < blks.size(); ++i)
            {
                GetPasses().GetPass<TerrainPass2>().UpdateBlockTexture(
                    assets, blks[i], i);
            }

            auto vpe = ui::CreateGameView(m_world,
                                          {math::vec2{0, 0}, math::vec2{1, 1}});
            m_player =
                ComponentPlayer::CreatePlayer(m_world, {20.f, 20.f, 20.f});
            {
        ...
        void AddWidgetInput(oge::runtime::AssetContext assets)
        {
            ...
            m_world.emplace<ui::UIRect>(mvWidget,
                                        math::vec2{0.0f, 1.f - scaledY},
        ...
        void Update(Frame f) override
        {
            ...
            GraphicalScene::Instance::Update(std::move(f));
        }
    };
    ...
    std::unique_ptr<Instance> Attach(const json::Value &args, OGEContext &ctx) override
    {
        ...
        return std::make_unique<DebugScene3Instance>(*this, args, ctx);
    }
};
}  // namespace game
