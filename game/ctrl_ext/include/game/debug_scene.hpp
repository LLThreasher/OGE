...
class DebugScene3 : public GraphicalScene
{
    ...
    void AddWidgetInput(oge::runtime::AssetContext assets)
    {
        ...
        m_inputs->AddStage<input::UIDragInput>(m_af);
        m_inputs->AddStage<input::WidgetInput>(m_af, m_widgetInputDef);
    }
   ...
   public:
                ...
                AnythingFactory& af) override
    {
        ...
        m_renderers->AddStage<view::TerrainRenderer>(m_af);
        m_renderers->AddStage<view::DebugInfoRenderer>(m_af);
        m_renderers->AddStage<view::UIRenderer>(m_af);
        m_renderers->AddStage<view::CameraRenderer>(m_af);
        ...
        // m_terminalButton = ui::CreateButton(world, context, {math::vec2{0.f,
        // 0.f}, math::vec2{0.1f, 0.1f}});

        // auto font = assets.LoadASCIIBitmapFont16x6("om_large_plain_idx.png");

        // {
        //     auto e = world.create();
        //     world.emplace<UIRect>(e, math::vec2{0.3f - 0.01f, 0.3f - 0.01f *
        //     context.backend.SwapchainAspect()}, math::vec2{0.1f, 0.1f *
        //     context.backend.SwapchainAspect()}); world.emplace<UISprite>(e,
        //     crossSprite); world.emplace<UIZLevel>(e, 1);
        // }

        // {
        //     auto e = world.create();
        //     world.emplace<UIRect>(e, math::vec2{0.3f - 0.01f, 0.7f - 0.01f *
        //     context.backend.SwapchainAspect()}, math::vec2{0.1f, 0.1f *
        //     context.backend.SwapchainAspect()}); world.emplace<UISprite>(e,
        //     crossSprite);
    ...
    void Update(Frame f) override
    {
        ...
        if (keys.contains(KeyCode::KY_G) && !usingKeyMouse)
        {
            ...
            m_inputs->AddStage<input::KeyMouseInput>(m_af, input::KeyMouseInput::Def{
                .target = m_world.get<input::PlayerInputStream>(m_player),
                .mouseIdx = 0,
