...
namespace OneGame::Engine::Graphics
{
    ...
    bool RingStagingBuffer::TryAllocate(
        uint64_t size, StagingAllocation& out)
    {
        ...
        if (m_head >= m_tail)
        {
            if (m_head + size > m_capacity &&
                size > m_tail)
            {
                //LOG_DEBUG("TryAlloc failed: {}, {}, {}", m_pendingFrees.size(), m_head, m_tail);
                return false;
            }
        }
        else
