...
void ChunkAllocator::Initialize(IGraphicsBackend* backend) { gpuBuffer = backend->CreateBuffer(bufferDesc); }

void ChunkAllocator::Shutdown(IGraphicsBackend* backend) { backend->DestroyBuffer(gpuBuffer); }
...
void ChunkAllocator::FreeChunk(uint32_t idx) { m_freeList.push_back(idx); }
}  // namespace OneGame::Engine::Graphics
