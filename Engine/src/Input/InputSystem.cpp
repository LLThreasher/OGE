...
void InputSystem::SetKey(KeyCode key, bool down) { m_currentKeys[static_cast<uint32_t>(key)] = down; }
...
void InputSystem::SetTouchUpdate(int id, float x, float y) { m_currentTouch[id] = {x, y}; }
...
bool InputSystem::IsKeyDown(KeyCode key) const { return m_currentKeys[static_cast<uint32_t>(key)]; }
...
bool InputSystem::IsMouseDown(MouseButton button) const { return m_currentMouse[static_cast<uint32_t>(button)]; }
