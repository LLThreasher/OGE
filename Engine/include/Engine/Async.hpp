...
class FixedSizeFunction<RetTy(Args...), MaxSize>
{
   ...
   public:
    ...
    RetTy operator()(Args... args) const { return thunk_ptr((void*)(storage), std::forward<Args>(args)...); }
};
...
{
   public:
    Future(FutureHandle h, AsyncDispatcher* d) : handle(h), dispatcher(d)
    {
    ...
    void post(T val) { dispatcher->post_future(handle, std::bit_cast<uint64_t>(val)); }
